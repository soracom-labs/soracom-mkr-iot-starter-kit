...
#define BUTTON_PIN 4   // MKR Connection carrier pin D4
...
volatile static bool isr_buttonPress = false;
static unsigned char buttonPressCounter = 0;
// TODO(JON) : CODE-CUT, explain 'volatile', 'static' & 'char' in the context of an ISR

void isr_button()
{
  isr_buttonPress = true;
}
...
String getDistance(String &separator)
{
  ...
  // Output sensor data to serial
  if (Serial)
    Serial.println("  Distance: " + String(d) + (ULTRA_UNIT == 0 ? " cm" : " inches"));
...
String getButton(String &separator)
{
  // Output sensor data to serial
  if (Serial)
    Serial.println("  Button: " + String(buttonPressCounter));

  String data = separator + "\"button\":" + String(buttonPressCounter);
  separator = ",";
  return data;
}
...
String getMagnetic(String &separator)
{
  ...
  // Output sensor data to serial
  if (Serial)
    Serial.println("  Magnetic Switch: " + String(m));
...
String getAccelerometer(String &separator)
{
  ...
  // Output sensor data to serial
  if (Serial)
    Serial.println("  Accelerometer: X=" + String(xyz[0]) + " g, Y=" + String(xyz[1]) + " g, Z=" + String(xyz[2]) + " g");
...
String getGps(String &separator)
{
  ...
  else
  {
    ...
    if (satellites != 0)
      data = data + ",\"gps_satellites\":" + String(satellites);
    if (hdop != 0)
      data = data + ",\"gps_hdop\":" + String(hdop, 1);
    if (latitude != 0)
      data = data + ",\"latitude\":" + String(latitude, 6);
    if (longitude != 0)
      data = data + ",\"longitude\":" + String(longitude, 6);
    if (altitude != 0)
      data = data + ",\"gps_altitude\":" + (validAltitude ? String(altitude / 1000., 3) : "\"not available\"");
    if (speed != 0)
      data = data + ",\"gps_speed\":" + String(speed, 3);
    if (course != 0)
      data = data + ",\"gps_course\":" + String(course, 3);
  }

  // Cleanup GPS data buffer
  if (Serial)
    Serial.print(">>> Cleaning up GPS data buffer...");
  nmea.clear();
  if (Serial)
    Serial.println(" done!");
...
void setup()
{
  ...
  if (Serial)
    Serial.println(">>> Starting Soracom MKR GSM 1400 demo with Grove Sensor Kit!");

  // Start button & attach the ISR
  ...
  if (ENABLE_GPS)
  {
    ...
    {
      if (Serial)
        Serial.println(" error, could not start Serial1 interface!");
    }
    else
    {
      while (gps.available())
        gps.read(); // Clear the buffer
      ...
      while (millis() < timeout)
      {
        while (gps.available())
        {
          char c = gps.read();
          if (nmea.process(c))
            gpsOnline = true; // Reset is complete when the first valid message is received
        }
        if (gpsOnline)
          break;
      }
      ...
      {
        if (Serial)
          Serial.println(" error, could not reset the GPS module!");
      }
      else
      {
  ...
  {
    if (Serial)
      Serial.println(" error, could not turn on modem! Try power-cycling.");
    return;
  }

  // Connect to the modem's serial interface
  if (Serial)
    Serial.print(">>> Connecting to modem...");
  SerialGSM.begin(baud);
  while (!SerialGSM)
  {
    ;
  } // Wait until the modem's serial interface is ready
  if (Serial)
    Serial.println(" connected!");
  ...
  if (Serial)
    Serial.print(">>> Connecting to 3G network...");
  SoraGprsConnect(); // Attach to the data network

  if (Serial)
    Serial.println(">>> Startup complete! Now reading sensors and sending data to " + String(HTTP_HOST));
}
...
void loop()
{
  ...
  if (Serial)
    ...
    if (isr_buttonPress)
    {
      isr_buttonPress = false;
      buttonPressCounter = buttonPressCounter + 1;

      Serial.println(">>> Button press detected, event number : " + String(buttonPressCounter, DEC));
    }
  ...
  {
    if (Serial)
      Serial.println(">>> Modem is still connected to 3G network!");
  ...
  {
    if (Serial)
      Serial.print(">>> Disconnected from 3G network. Reconnecting...");
    gsmAccess.shutdown(); // Reset the modem
    SoraGprsConnect();    // Attach to the data network
  }

  if (ENABLE_BUTTON || ENABLE_DHT || ENABLE_ULTRA || ENABLE_MAGNETIC || ENABLE_ACCEL || ENABLE_DHT)
  {
    if (Serial)
  ...
  // Connect to server
  if (Serial)
    Serial.print(">>> Connecting to " + String(HTTP_HOST) + "...");
  ...
  SoraLoopCounter++;
  if (Serial)
    Serial.println(">>> Waiting " + String(SEND_INTERVAL) + " seconds until next reading");
